...
my $command = "bnet";
my $access = 0; # Public
my $description = "This is a bnet command for building new actual commands";
my $pattern = '^(~bnet)\s?([a-zA-Z0-9#]+)?\s?([a-zA-Z0-9]+)?';
my $function = \&cmd_bnet;
my $usage = <<EOF;
~bnet
EOF
###########################################################################################
...
sub cmd_bnet
{
    ...
    if(index($search, "#") != -1) { 
        ...
        if($racial =~ /^$/) {
            ## just bnet no race
            $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `battle_tag` LIKE '%".$search."%' ORDER BY `mmr` DESC");
        }
        else {
            $search =~ s/$racial//g;
            $search =~ s/ //g;
            if($racial =~ /^(terran|zerg|protoss|random)$/i) {
                $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `battle_tag` LIKE '%".$search."%' AND `race` = '".$racial."' ORDER BY `mmr` DESC");
            }
            elsif($racial =~ /^(kr|na|us|eu)$/i) {
                if($racial =~ /^na$/) {
                    $racial = "us";
                }
                $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `battle_tag` LIKE '%".$search."%' AND `server` = '".$racial."' ORDER BY `mmr` DESC");
            }
            else {
                MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "please use a race or server: ~bnet shortland#56600 **NA**"}', "1", "");
                return;
            }
        }

        #my $sth = $dbh->prepare("SELECT * FROM `everyone` WHERE `battle_tag` = '".$Pbattle_tag."' ORDER BY `mmr` DESC");
        ...
        if($racial =~ /^(terran|zerg|protoss|random)$/i) {
            ...
            MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "please use a race or server: ~bnet shortland#56600 **NA/Terran**"}', "1", "");
            return;
        }
        goto HERO;
    }
        ...
        # $text = "There may be multiple people (ranked in multiple races) with a similar battle_tag found, \\nPlease be more specific by using their SC2 tag. \\nie: ~bnet Shortland**#952**\\n" if ($amt > 1);
        if ($amt > 1) {
            # print in chunks
            my $mess = "";
            my $j = 0;
            ...
            MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "There are multiple people (ranked in multiple races) with a similar battle_tag found, \\nPlease be more specific by using their SC2 tag and race. \\nie: ~bnet Shortland**#952** **terran**\\n"}', "1", "");
            ...
            MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "unable to find that person"}', "1", "");
        }
    }
}
